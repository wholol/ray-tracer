...
struct comparator		//used to determine priority queue order.
{
	...
	bool operator()(BVH_node* left, BVH_node* right) {
		return (left->box.tmin_magnitude() > right->box.tmin_magnitude());
	}
};
...
public:
	...
	void traverseBVH_parallel(const Ray& r, double tMin, double& tMax, hit_point& hitpoint , BVH_node* node,bool& hit);
